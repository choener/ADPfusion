...
data Backtrack t r where
  Backtrack :: !t -> !(forall m i . i -> i -> m (S.Stream m r)) -> Backtrack t r
-}
...
-- Note that the index type of @arr@ need not be the same as the index type
-- of the DP
...
--   mkStream (ls :!: MTbl c t _) Static lu (Subword (i:.j))
--     = S.mapM (\s -> let Subword (_:.l) = getIdx s
--                     in  PA.readM t (subword l j) >>= \z -> return $ ElmMTbl z (subword l j) s)
--     $ mkStream ls (Variable Check Nothing) lu (subword i $ j - minSize c)
--   mkStream (ls :!: MTbl c t _) (Variable _ Nothing) lu (Subword (i:.j))
...
--           {-# INLINE [1] mk   #-}
--           {-# INLINE [1] step #-}
--       in  S.flatten mk step Unknown $ mkStream ls (Variable NoCheck Nothing) lu (subword i j)
--   {-# INLINE mkStream #-}
-- 
-- instance
--   ( Monad m
...
--   mkStream (ls :!: BtTbl c t f) Static lu (Subword (i:.j))
--     = S.map (\s -> let Subword (_:.l) = getIdx s
--                        ix             = subword l j
--                        d              = t PA.! ix
--                    in  ElmBtTbl d (f ix) ix s)
--     $ mkStream ls (Variable Check Nothing) lu (subword i $ j - minSize c)
--   mkStream (ls :!: BtTbl c t f) (Variable _ Nothing) lu (Subword (i:.j))
--     = let mk s = let (Subword (_:.l)) = getIdx s in return (s:.j-l-minSize c)
--           step (s:.z)
--             | z>=0      = do let (Subword (_:.k)) = getIdx s
--                                  ix               = subword k (j-z)
...
instance
  ...
  mkStream (ls :!: Backtrack (ITbl c t _) bt) Static lu (Subword (i:.j))
    = S.mapM (\s -> let Subword (_:.l) = getIdx s
                        ix             = subword l j
                        d              = t PA.! ix
                    in  return $ ElmBtITbl d (bt lu ix) ix s)
    $ mkStream ls (Variable Check Nothing) lu (subword i $ j - minSize c)
  mkStream (ls :!: Backtrack (ITbl c t _) bt) (Variable _ Nothing) lu (Subword (i:.j))
    = let mk s = let (Subword (_:.l)) = getIdx s in return (s, j - l - minSize c)
          step (s,z) | z>=0 = do let (Subword (_:.k)) = getIdx s
                                     ix = subword k (j-z)
                                     d = t PA.! ix
...
--   mkStream (ls :!: MTbl c t _) vs lu is
...
--   mkStream (ls :!: BtTbl c t f) vs lu is
...
instance (PA.ExtShape i, PA.PrimArrayOps arr i x) => Axiom (Backtrack (ITbl m arr i x) r) where
  ...
  axiom (Backtrack (ITbl _ arr _) f) = f (error "missing bounds") . uncurry topmostIndex $ PA.bounds arr
  {-# INLINE axiom #-}
-}
