...
instance
  ...
  addIndexDenseGo (cs:.c) (vs:.IStatic rp) (us:.u) (is:.i)
    ...
    where mk ixS
            ...
            | otherwise  = return $ Just (ixS :. mask :. k)
            where k  = (BitSet $ 2^cm-1)
                  cm = popCount mask - rp
                  mask = i `xor` l
                  l = getIndex (sIx ixS) (Proxy :: Proxy (is:.BitSet I))
          step Nothing = return $ Done
          step (Just (ixS@(IxS s a b y z y' z') :. mask :. k))
            | pk > popCount i - rp = return $ Done
            | otherwise            = let kk = popShiftL mask k
                                         aa = getIndex a (Proxy :: Proxy (is:.BitSet I))
                                     in  return $ Yield (IxS s a b (y:.kk) (z:.0) (y':.(kk.|.aa)) (z':.0))
                                                        ((ixS :. mask :.) <$> setSucc 0 (2^pk -1) k)
            where pk = popCount k
  ...
  addIndexDenseGo (cs:.c) (vs:.IVariable rp) (us:.u) (is:.i)
    = flatten mk step . addIndexDenseGo cs vs us is
    where mk ixS
            | c==EmptyOk  = return $ Just (ixS :. mask :. cm :. 0)
            | cm == 0     = return $ Nothing
            | c==NonEmpty = return $ Just (ixS :. mask :. cm :. 1)
            where mask = i `xor` l
                  cm   = popCount mask
                  l    = getIndex (sIx ixS) (Proxy :: Proxy (is:.BitSet I))
          step Nothing = return $ Done
          step (Just (ixS@(IxS s a b y z y' z') :. mask :. cm :. k))
            | popCount i < popCount (kk .|. l) + rp = return $ Done
            | otherwise = return $ Yield (IxS s a b (y:.kk) (z:.0) (y':.(kk.|.l)) (z':.0))
                                         ((ixS :. mask :. cm :.) <$> setSucc 0 (2^cm -1) k)
