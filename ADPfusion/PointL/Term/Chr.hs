...
type instance LeftPosTy (IStatic   '(low,high)) (Chr r x) (PointL I) = IStatic '(low+1,high+1) -- Chr just reduces the PointL by 1
type instance LeftPosTy (IVariable '(low,high)) (Chr r x) (PointL I) = TypeError (Text "This should be allowed? Only disallowed if Left of synvar")
...
-- | Actual handling of multidimensional parsing, which is re-used for the single-dim case.

instance
  ( TermStreamContext m ps ts s x0 i0 is (PointL I), KnownNat low
  ) => TermStream m (ps:.IStatic '(low,high)) (TermSymbol ts (Chr r x)) s (is:.PointL I) where
--{{{
  termStream Proxy (ts:|Chr f xs) (us:..LtPointL u) (is:.PointL i)
    ...
    = S.map (\(TState s ii ee) -> let !e = f xs $! i-1-low in TState s (ii:.:RiPlI i) (ee:.e))
