...
type instance LeftPosTy (IStatic '(low,high)) (Epsilon Global) (PointL I) = IStatic '(low,high)
type instance LeftPosTy (IStatic '(low,high)) (Epsilon Local) (PointL I) = IVariable '(low,high)  -- to actually allow local epsilons to work, IStatic does additional static controls
--type instance LeftPosTy (IVariable d) Epsilon (PointL I) = IVariable d

type instance LeftPosTy (OStatic '(low,high)) (Epsilon Global) (PointL O) = OStatic '(low,high)
...
instance
  ...
  => TermStream m (ps:.IStatic '(low,high)) (TermSymbol ts (Epsilon lg)) s (is:.PointL I) where
...
instance
  ...
  ) => TermStream m (ps:.OStatic '(low,high)) (TermSymbol ts (Epsilon lg)) s (is:.PointL O) where
  termStream Proxy (ts:|Epsilon) (us:..LtPointL u) (is:.PointL i)
    = S.map (\(TState s ii ee) ->
...
instance TermStaticVar (IStatic '(0,0)) (Epsilon Global) (PointL I) where
  termStreamIndex Proxy Epsilon (PointL i     ) = PointL i
  termStaticCheck Proxy Epsilon _ (PointL (I# i)) grd = (i ==# 0#) `andI#` grd
  {-# Inline [0] termStreamIndex #-}
  {-# Inline [0] termStaticCheck #-}

instance TermStaticVar (IStatic '(0,0)) (Epsilon Local) (PointL I) where
  termStreamIndex Proxy Epsilon (PointL i     ) = PointL i
